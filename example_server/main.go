...
var reqLogTextTemplate = template.Must(template.New("meta_nouns_text").Parse(`
{{- define "item" -}}
{{ .method }} {{ .URL }}
{{end -}}
`))
...
func main() {
	rl := logged(http.DefaultServeMux)
	gwr.AddMarshaledDataSource(rl)
	log.Fatal(http.ListenAndServe(":4040", rl))
}
